...
def submit_assessment(
    ...
    if existing_assessment:
        ...
        existing_assessment.biggest_obstacle = payload.biggest_obstacle
        db.commit()
...
def assessment_status(
    ...
    report = (
        ...
        .filter(PersonaReport.user_id == current_user.id)
        .first()
    )

    return AssessmentStatusResponse(
        assessment_completed=assessment is not None,
        is_persona_setup_completed=persona is not None,
        is_report_generated=report is not None,
    )
